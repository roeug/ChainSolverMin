...
function TChainCalculator.GetN0(StateNo: Integer): double;
begin
 Result:= fNvsTimeST[StateNo, 0];
end;

procedure TChainCalculator.SetN0(StateNo: Integer; Value: double);
begin
...
{$IFDEF WINDOWS}
 {$IFDEF CPU32}
 DllFileNameToLoad:= 'mebdf';
 {$ELSE}
 DllFileNameToLoad:= 'mebdf64';
 {$ENDIF}
{$ENDIF}
{$IFDEF LINUX}
 {$IFDEF CPU32}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'mebdf.so';
 {$ELSE}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'mebdf64.so';
 {$ENDIF}
{$ENDIF}
 HDLL:= LoadLibrary(DllFileNameToLoad);
 if HDLL >= 32 then
...
// TolSwitch
 ...
 begin
  MessageDlg('MEBDF.DLL "' + DllFileNameToLoad +'" not found', mtError, [mbOk], 0);
  Result:= -1;
 end;
end;
...
{$IFDEF WINDOWS}
 {$IFDEF CPU32}
 DllFileNameToLoad:= 'radau';
 {$ELSE}
 DllFileNameToLoad:= 'radau64';
 {$ENDIF}
{$ENDIF}
{$IFDEF LINUX}
 {$IFDEF CPU32}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'radau.so';
 {$ELSE}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'radau64.so';
 {$ENDIF}
{$ENDIF}
 HDLL:= LoadLibrary(DllFileNameToLoad);
 if HDLL >= 32 then
...
// TolSwitch
 ...
 begin
  MessageDlg('RADAU.DLL "' + DllFileNameToLoad +'" not found', mtError, [mbOk], 0);
  Result:= -1;
 end;
end;
...
{$IFDEF WINDOWS}
 {$IFDEF CPU32}
 DllFileNameToLoad:= 'vode';
 {$ELSE}
 DllFileNameToLoad:= 'vode64';
 {$ENDIF}
{$ENDIF}
{$IFDEF LINUX}
 {$IFDEF CPU32}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'vode.so';
 {$ELSE}
 DllFileNameToLoad:= ExtractFilePath(Application.ExeName) + 'vode64.so';
 {$ENDIF}
{$ENDIF}
 HDLL:= LoadLibrary(DllFileNameToLoad);
   if HDLL >= 32 then { OK }
...
// TolSwitch
   ...
   MessageDlg('VODE.DLL "' + DllFileNameToLoad +'" not found', mtError, [mbOk], 0);
  end
  else
  begin //not DLL
   try
...
function TChainCalculator.GetStateThZpA_s(Index: Integer): integer;
begin
 if Index < fChain.States.Count then
 try
  Result:= fChain.States[Index].ThZpA_s;
 except
  Result:= -1;
 end
 else
...
function TChainCalculator.GetFissionEnergyDepositionNIvsTime(const StateNo, TimeNo: integer; const ConsiderDepression, SSKconsider: Boolean; const Energy4fission: double): double;
var
 I: integer;
 N_nuclei, l_mul_MacroSigma, MacroSigma: double;
 SigmaF, RIf, SigmaF_fast, aThermalFlux, aResonanceFlux, aFastFlux, aTng: double;
begin
...
function TChainCalculator.GetElementMass(const ElementName: string; const TimeNo: integer): double;
var
 I: integer;
...
function TChainCalculator.GetStateMass(const StateNo, TimeNo: Integer): double;
begin
 Result:= fNvsTimeST[StateNo, TimeNo] / N_Av * (((Self.fChain.States[StateNo].ThZpA_s) div 10) mod 1000);
end;

function TChainCalculator.GetTotalMass(const TimeNo: Integer): double;
var
 I: integer;
begin
 Result:= 0;
...
function TChainCalculator.GetDepresssionK(const TimeNo: integer): double;
var
 NoOfY: integer;
...
function TChainCalculator.CalcSSK(const StateNo: integer; const Nuclei: double): double; //==ApproximateSSK
var
 fN, fSSK: TFloatList;
 i, k: integer;
 N, aN, dYdX: double;
...
function TChainCalculator.GetAtolerance(Index: integer): double;
begin
 Result:= fatolerance[Index];
end;

function TChainCalculator.GetRtolerance(Index: integer): double;
begin
 Result:= frtolerance[Index];
end;

procedure TChainCalculator.SetAtolerance(Index: integer; Tol: double);
begin
 fatolerance[Index]:= Tol;
end;

procedure TChainCalculator.SetRtolerance(Index: integer; Tol: double);
begin
...
procedure TSSK_Table.Add(const aConcentration, aSSK: double);
begin
...
function TSSK_Table.Get_Concentration(Index: integer): double;
begin
 Result:= fConcentration[Index];
end;

function TSSK_Table.Get_SSK(Index: integer): double;
begin
 Result:= fSSK[Index];
end;

(*
function TSSK_Table.InterpolateSSK(const aConcentration: double): double;
var
 i, k: integer;
 dYdX: double;
begin
...
function TSSK_TableList.InterpolateSSKforStateName(const aConcentration: double;
...
function TSSK_TableList.InterpolateSSKforStateNo(const aConcentration: double;
 const StateNo: integer): double;
begin
// No Checks - for speed
...
procedure TSSK_Table.Set_Concentration(Index: integer; aConcentration: double);
begin
 fConcentration[Index]:= aConcentration;
end;

procedure TSSK_Table.Set_SSK(Index: integer; aSSK: double);
begin
 fSSK[Index]:= aSSK;
end;
